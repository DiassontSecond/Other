class HeartGLRender {
    ...
    constructor(el) {
        ...
        if (!this.gl) throw new Error('WebGL не підтримується вашим браузером або вимкнено.');
    ...
    static createPositionHandler(gl, program) {
        ...
        if (attributeLocation === -1) {
            throw new Error('Атрибут "position" не знайдений в шейдері.');
        }
...
function changePhoto() {
    // Затухаємо всі зображення
    photos.forEach(img => {
        img.style.transition = 'opacity 0.5s ease';
        img.style.opacity = 0;
    });

    // Після затухання, змінюємо зображення та відновлюємо їх
    setTimeout(() => {
        photos.forEach(img => {
            img.src = `Img/photo${(currentPhotoIndex % 12) + 1}.jpg`;
            img.style.transition = 'opacity 1s ease';
            img.style.opacity = 1; // Знову робимо їх видимими
        });

        currentPhotoIndex++;
    }, 500); // Чекаємо 500 мс перед зміною
}
